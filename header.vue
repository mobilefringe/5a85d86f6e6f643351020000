    					...
    					<nav id="primary_nav">
    						<ul>
    						    <li v-for="item in menu_items" class="menu_item dropdown" :class="{open : item.open_dropdown}">
    						        <router-link v-if="item.sub_menu" to="#" data-toggle="dropdown" class="dropdown-toggle" aria-haspopup="true" :aria-expanded="item.open_dropdown" @keyup.down="item.open_dropdown = true; console.log('');">{{$t(item.name)}}</router-link>
