                    ...
                    <form class="form-horizontal padding_top_20 js-cm-form" v-on:submit.prevent="validateBeforeSubmit" id="kids_pine_club" method="post">
						...
						<div class="form-group">
    						<!--<div class="col-sm-6 col-xs-12" :class="{'has-error': errors.has('timeslot1')}">-->
                				<v-select :options="timeslot" label="name" :searchable="false" :on-change="timeSlotOneSelected()" id="" placeholder="Select first choice" v-validate="'required:true'" class="form-control" :class="{'input': true}"></v-select>
                				<span v-show="errors.has('email')" class="form-control-feedback">{{ errors.first('email') }}</span>
                			<!--</div>-->
